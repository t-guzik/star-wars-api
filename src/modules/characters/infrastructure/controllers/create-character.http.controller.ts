...
export class CreateCharacterHttpController {
  ...
  @ApiResponse({status: HttpStatus.CONFLICT, description: CharacterAlreadyExistsException.message, type: ApiErrorResponse})
  ...
  async createCharacter(@Body() body: CreateCharacterRequestDto): Promise<IdResponse> {
    try {
      const result = await this.commandBus.execute<CreateCharacterCommand, AggregateID>(new CreateCharacterCommand(body));
